...
| 节点 display 语义（v0.1.00017） | 一个 net 仅一个节点可点亮 display（节点右上角蓝色 D chip，点击切换唯一）；要显示多个结果必须手动加 merge 节点（Houdini 语义） | 与 Houdini SOP 一致：display 是排他的视图开关，不是 per-node 布尔堆积；视口显示由「当前 display 节点」决定（input_/null → inputs 组，output_ → outputs 组） |
