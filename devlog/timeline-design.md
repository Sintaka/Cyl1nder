...
> **状态注记（v0.1.00104，2026-08-14）**：双向同步的**实现通道已换**——本设计的 `/pending` 捎带 + kick 方案未采用；实际实现走官方 fxhoudinimcp（C→H `animation.set_frame`、H→C 常驻轮询 + WS 推送，见 `houdini-mcp-integration.md` / `timeline-sync-lag-analysis.md`）。**engaged 锚定门控仍未实现**（现门控 = `linkEnabled`（mcpPort>0）+ 拖动态回显抑制），是吊牌/项目阶段可选的补强项。
