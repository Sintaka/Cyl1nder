...
> **状态（v0.1.00101，2026-08-14）**：**Phase B 已实现并合并**——`syncEnabled`（web 单一事实源，prefs 默认 OFF）+ 底部栏 Sync 开关（`#cyl-sync-enabled`，右下角）；bridge per-serial `sync_enabled`（默认 False，`PUT /sync-enabled`；OFF 不广播回显/不 notify_stream；`/pending`、`/stream` 事件携带，`/status` 返回 sync 块）；HDA 自适应循环（OFF ~1.5s `/pending` 探测 gate 兼心跳、**零 /stream 请求**；ON 跑 `/stream`；事件 `sync_enabled=false` 切回探测）；web network.shouldPush / session 忽略回显 / kick / viewport-edit 全 gate。验证：pytest 61 / tsc 0 / vitest 160 / hython SMOKE OK。**下一轮做 Phase C（IndexedDB 帧缓存）**。
> **状态（v0.1.00103，2026-08-14）**：**时间轴双向同步已按 fxhoudinimcp 通道落地**（本计划 Phase A/B 的「HDA 锚定 /stream 方案」被 00102/00103 的官方 MCP 方案取代）——C→H：web scrub 节流提交 → bridge `animation.set_frame`；H→C：bridge 常驻轮询 + WS 帧变化推送；速率全部由 Sync Max FPS 派生；卡顿根因与通道上限见 `timeline-sync-lag-analysis.md`。**Phase C（IndexedDB 帧缓存）仍待做**（gate = A+B 已满足）。
