...
## v0.1.00108（2026-08-15）——P2b 项目模式（session 多开 + nodeview 项目根 + boot 粘合）
- **core/session.ts 重构为 SessionManager**（写集 A）：`ensureSession`（幂等：putSyncFps→putSyncEnabled→startHdaWatch→connectWs）/`activateSession`（ensure+setSerial+bumpEpoch+loadSnapshot，不重建 WS）/`closeSession`（拆线+停 watchdog，关活动成员清空活动）；消息按活动成员路由（活动 4 分支原样、后台仅簿记）；SessionDeps 仅 `startHdaWatch` 返回 stop + 可选 `connectWsFn`。**lifecycle.ts** watchdog 多槽（`Map<serial,stopFn>`）。测试 +13。
- **nodes2 项目根**（写集 B）：NodeKind +`"project"|"channel"`；CylNode.channel；makeProjectNode（0 端口）/makeChannelNode（1/1）；序列化自动判定 **schemaVersion 3**（含新 kind，null 省略）vs **2**（纯旧 kinds 绝不含新字段）；restoreGraph v3 分支 + 未知 kind 跳过；getNetworkSnapshot 过滤新 kind；planProjectGraph（param 跳过/saved 位置恢复/缺失成员排除）；graph.ts 独立 channel display 状态机（不碰 flags.display）+ 模块级与 ReteGraph 实例双导出 `setChannelDisplayHandler/getChannelDisplaySerial/isProjectMode/loadProjectGraph/projectGraphSnapshot`；NodeView project/channel 分支；PALETTE 不可建；chain-cache 对新 kind 返回死链；dataflow 过滤。测试 +14。
