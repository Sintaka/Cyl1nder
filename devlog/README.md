...
- **v0.1.00118**：**dot 从节点降级为连线装饰件 + 双击缩放拦截**——进度真相见 [in-progress.md](in-progress.md)。
  ...
  **bug A 双击缩放**：根因是 rete `Zoom` 自带 `dblclick` 处理器（不是我们调的）。`zoom` 是可取消的 guard 管道事件且 `ZoomEventParams` 带 `source`，于是只拦 `source === "dblclick"`——滚轮与程序化 `zoomAt` 照旧放行。这是 #4「双击进入节点」的前置。
  **bug B Alt 手势**：`Alt+左键`点线生成 waypoint、拖动跟手、甩远 112px 删除。**Alt 从未被浏览器或系统屏蔽**，是我们自己的处理器 `if (e.altKey) return` 忽略它。关键顺序：Alt 分支必须在 `trackedConnId` 赋值**之前**拦截，这是「不出转接预览、线不进选中态」的全部原因。
  **只有真浏览器能发现的 bug**：`pointermove` 挂在 `container`（图面板）而非 window，指针一离开面板就收不到事件——20 步甩动只有 3 个 move 到达，距离永远够不着阈值，**甩远删除完全不触发**，而此时 tsc 0 / vitest 全绿。修法是手势期间额外在 window 挂一份 capture 监听，按事件对象身份去重。
  **`as` 强转会让类型删除静默失效**：`dot-neutral.test.ts` 用 `as NetworkSnapshot` 塞 `kind:"dot"` 节点，所以 `"dot"` 移出联合类型后它照样编译、照样全绿——已在验证一个不存在的行为。改写为 `waypoint-neutral.test.ts`，驱动真实 `NodeEditor` 验证结构层面的不可能出错。
  验证：tsc 0 / vitest **582**（34 文件；579 − 旧 `dot-neutral` 2 条 + 新 `waypoint-neutral` 5 条）/ `vite build` 通过 / `waypoint-verify` **4 passed** / `round18-reconnect` **5 passed**（后两项在 Edge 真实浏览器，桥 8375 + vite 8376 在线）。
...
- v0.1.00114 → **v0.1.00115**：**项目化重构（项目优先 + 映射系统 + 类型化端口 + 存活实证）**——一轮跨两个版本号，权威说明见 [project-mapping-design.md](project-mapping-design.md)。
  ...
  **存活实证**：心跳只证明「最近 cook 过」（吊牌长期不 cook 是正常的），所以锚点记 `pid`+`mcpPort`，降级前打 `mcp.health` 核对 pid——`alive && pidMatched` 才算活着，`alive && !pidMatched` = 端口被另一个 Houdini 占了必须判失联。心跳 5s → **60s**。
  **overview 项目优先**：项目置顶（行内改名/删除/清理空项目/聚合状态灯）→ 映射表（逻辑名/类型/绝对路径/行内改值/解绑）→ 场景降级为默认折叠诊断区；「序列号尾巴」修复（label 是序列号或等于成员 serial → `未命名项目 · <成员名>`，serial 只进 title）。
  **nodeview**：`geo`/`float`/`vec3` 类型化 socket + 连线类型校验（此前 `ConnectionPresets.classic.setup()` 来者不拒）+ 按类型着色（对齐 Houdini VOP）；`_input_`/`_output_` 单端口 + `address`；多 out 报错不静默取第一条；新增 `ADDRESS_GRAPH_SCHEMA=4`（v2/v3 输出字节不变，旧图仍走 4 端口）。
  **三个死代码/错假设（都是「看起来能用」那类）**：① `mcp.health` **不回 hip_file**，`discover_by_hip` 恒为 None → 改按 **pid** 扫 8100..8115（附带自愈端口）；② 吊牌**不在 registry.json 里**，`_resolve_port(tagSerial)` 会掉到 `discover_first()`「拿第一个活口当答案」（多开 Houdini 必错）；③ 项目状态灯读的是成员**冻结快照**的 lastSeen（实测快照 13081s vs live 2938s）→ 改读 `/api/channels` live 表。
  验证：pytest 270、tsc 0、vitest 421、e2e 87 passed/1 skipped、实机 8100（改名吊牌后逻辑名不变+值仍可读；端口写错自愈；foreign pid 被拒）。
...
- v0.1.00109：**P3 轨迹页（谁动了数据）**——bridge：TraceStore（内存环形 10000、Lock、add 吞异常零行为影响）+ GET /api/trace（actor/action/channel/target/limit 过滤、project 按成员关系解析）+ 6 处埋点（inputs-push/outputs-edit×2/param-set|expr-set|python-exec/register|heartbeat）；web：/trace.html 审计视图（项目/actor/action/channel 过滤、actor 6 色徽标、行展开 digest、?project= 初始过滤）+ client/types 镜像 + vite 多入口；协议三处同步；2 子智能体并行+主进程合并（main.py 挂载）；验证：pytest 187、tsc 0、vitest 244、实机 8100 审计链闭环（runtime 改参→param-set→吊牌 cook→heartbeat 全在轨迹可见，?project= 过滤命中）。P4 延伸通道远期。
- v0.1.00108：**P2b 项目模式（多 HDA 绑定第二步）**——web：session 重构为 SessionManager（单活动成员+后台多会话，watchdog 多槽）+ nodeview 项目根（project/channel 新 kind、schemaVersion 3 项目图、channel display 独立状态机）+ main.ts 项目模式 boot/导航/保存分流 + ?project= 放行；bridge：项目图端点 GET/PUT /api/projects/{id}/graph（data_dir/projects/<pid>/graph.json + 单成员隐式项目迁移读）；协议三处同步；4 子智能体并行+主进程合并（round8 类名冲突 + round10 Connect 重连语义两处修复）；验证：pytest 169、tsc 0、vitest 229、e2e 86 passed+1 skipped 全量回归、实机 8100 项目模式浏览器级验收（项目根渲染/通道点击激活/项目图往返）。P3 轨迹页下轮。
- v0.1.00107：**P2a 项目层（多 HDA 绑定第一步）**——bridge：P1- 项目序列号 + ProjectRef + projects.py（projects.json 落盘）+ project_routes.py 6 端点（建/列表/查/加成员/移成员/ensure 隐式项目）；web：types/client 镜像 + stores/projects.ts + overview 项目区块（新建/展开/成员打开与移除 + HTML5 拖通道入项目 + ?project= 聚焦）+ main.ts boot 后台 ensure；协议三处同步；2 子智能体并行+主进程合并（main.py 挂载）；dev 规范补「禁止随意杀进程」铁律（桥重启走 shelf `cyl1nder::reload_bridge` 按端口操作）+ Invoke-RestMethod 中文 body Latin-1 坑；验证：pytest 157、tsc 0、vitest 202、实机 8100 全端点往返。P2b（nodeview 项目根+多 serial WS）下轮。
