...
- **一轮对话的终点**：用户交代的任务做完 → **报告用户 → 结束**。不要自己找下一个任务；不确定就问用户。
  `devlog/agent-calibration.md` 是**查阅型**经验清单（派活前、或撞到"这错好像犯过"时才翻），**不是每轮必读**。
...
- 测试基线（v0.1.00184，**实测**）：pytest **460** / vitest **972**（51 文件）/ tsc **0** / e2e **112 passed + 1 skipped**（2.1 分钟，全量跑完 teardown 自动扫掉合成项目，实测项目/serial/通道行三项均回到基线，零泄漏）。
  基线数字每轮都会变，**发现对不上时以实际跑出来的为准**，别信这一行（它只是给你一个量级参照）。

## 文档分四类：按**什么时候读**分，不按主题分（v0.1.00182 整理）

**A. 每轮必读（就这三份，约 20 KB，别再往里加）**
`AGENTS.md`（铁律）→ 本文件（入口 + 当前状态）→ `devlog/in-progress.md`（**只留最新 5 节，约 11 KB**）。
> 整理前这条链是 **291 KB**（`in-progress` 169 KB + `README` 65 KB + 规范 32 KB…），
> 每轮开工整份灌进上下文 —— 这是会话卡顿与 token 暴涨的直接原因。

**B. 按触发条件读（平时不要碰）**

| 触发条件 | 读哪份 |
|---|---|
| 要动 bridge/web/hda **任一端的线格式** | `protocol.md` |
| 写 `.mjs` 编排脚本 / 用 `spawnSync` 读输出 / 定退出码 | `sandbox-env-facts.md` |
| 要**派活**给子智能体 | `SUBAGENT_BRIEF.md`（给子智能体读）+ `agent-calibration.md`（哪些活不该派） |
| 碰 APEX / `sceneanimate` | `apex-runtime-knowledge.md`（含两条破坏性铁律）→ `apex-scene-animate-runtime.md` |
| 提交 / 发布 / 跑门禁 | 下方「固化脚本」表，**别手搓等价物** |
| 想确认开发规范 | `development-standards.md`（分支/版本/编码卫生/UI） |
| 吊牌 HDA 计划状态 | `tag-hda-plan.md`（P1~P5b 已完成；仍搁置：时间轴互补通道、data 值面板正式 UI） |
| 找某个专题文件 | `README.md` 字典 |

**C. 事后记录（**从不**在开工时读，只在考古时查）**
`archive/in-progress-history.md`（§-48 及更早）、`annotations-{web,bridge,hda}.md`（三份合计 187 KB）、
`tag-t-sync-latency.md` 等专题复盘。它们是**写给未来的**，不是读给当下的。

**D. 历史调研库 → `devlog/archive/`（25 份）**
zeno 系列、早期选型调研、已完成计划、版本历史。**任何一轮开发都不需要它们。**
...
| 卫生检查（新增行 + 工作树游离 BOM） | `node scripts/check-hygiene.mjs`（通过时**静默无输出**） |
...
- **每轮收尾跑一次 `node scripts/check-hygiene.mjs`**：查四类问题——写工具静默截断
  `[N chars omitted]`、变异测试标记残留、中文乱码（3+ 连续 `?`）、**工作树里夹在文件中间的
  游离 U+FEFF**（最后这条旧脚本查不到，实测靠人眼才发现）。
  `commit-step.mjs` 与 `verify-all.mjs --staged` 已内置调用它，单独跑只在收尾自查时需要。
  **通过时它不输出任何东西**（退出码 0 已说完）——没问题就不必回复用户，有问题才报。
  `0` 通过 / `1` 有问题 / `2` VACUOUS（无内容可查，不构成通过）/ `3` 工具没跑起来。
  不要手搓 grep 代替它：手搓那版一个会话误报三次，最后一次我在它打红后照样提交了 ——
  一个被训练成可以忽略的检查，比没有检查更坏。
- 文档改动：中文经 edit/write 工具或 UTF-8 无 BOM 写入（pwsh 7 下 `-Encoding utf8` 本来就无 BOM）。
  别用裸 `??` 抽查乱码 —— JS 的空值合并运算符和散文引用它都会命中；判乱码交上面那个脚本。
