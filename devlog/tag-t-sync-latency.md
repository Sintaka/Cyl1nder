...
| 9 | throttled 之后状态点**永久卡在 pending**（缺陷 8 修复的副作用） | **已确诊、未修**（下轮） | 连发 4 次 PUT：第 3、4 次实测回 `{"ok":true,"throttled":true}` |

> 第 9 项是**第 8 项修复自己带出来的**：00169 让 `throttled` 正确画成 pending（不再谎报绿），
> 但**没人负责把这个 pending 解除**。`setDot` 全仓只有 3 个调用点（用户改值 ×2、flush 结算 ×1），
> `applyValues`/`setRowValue` 完全不碰 dot —— 于是桥随后延迟 flush **写成功了**，
> 面板却一直显示「编辑中（待提交）」。
>
> **6 与 9 是一对镜像**：6 是**谎报成功**（写失败画绿），9 是**谎报未完成**（写成功却显示待提交）。
> 两者都让状态点失去意义 —— 一个状态指示器的价值在于**它会变**，卡住的绿点和卡住的黄点一样没用。
>
> 修法：延迟 flush 没有 HTTP 响应可挂（`houdini_routes.py` 注释已写明），
> 所以**用 250ms 轮询回来的值当确认信道** —— 值等于期望即视为已落地，解除 pending。
>
> **下轮实现时先看这条**（被打断的子智能体在自己初稿里抓到的顺序 bug，值钱）：
> `applyValues` 里必须**先取下期望值再调 `setRowValue`** —— `setRowValue` 会覆写
> `row.value`，若先覆写再比较，`pendingResolvedBy(row.value, incoming)` 就**恒为真**，
> 于是任何一次轮询都会把 pending 解除掉，等于没修。
> 这与 §-50「写路径的验证必须写一个与当前值不同的值」同型：
> **比较两边如果被同一次赋值污染，那个比较就没有可能失败。**
