...
| 6 | `PUT channel-values` 对失败的写**谎报成功** | **已修**（00168） | 逐通道回报 `failed`；变异测试 4 条红 |

> 第 6 项是查第 4 项的写侧时撞到的，**比第 4 项严重且不限于 vec3**：任何逐通道写失败
> （路径写错／参数被锁／类型不符）都被报成成功，而面板状态点只看 `r.ok`
> —— 用户看到绿色「已同步」，写却没落地。详见 in-progress §-41。
>
> **仍未做（明细，非阻断）**：`client.ts` 只读 `error` 不读 `failed`，所以点会正确变红、
> 但顶部横幅显示「提交失败：unknown」，看不出是哪一条通道。我一度以为这条是阻断性的
> （绿点不会变红），读到 `client.ts:578` 的 `ok: body?.ok ?? true` 才发现 `??` 只兜
> null/undefined，`ok:false` 原样穿过 —— 更正记在 §-41。
