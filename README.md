...
版本：`0.1.00063`（x.xxx.xxxxx，每次 commit dailybuild++，规范见 devlog/development-standards.md）。当前架构：HDA 事件驱动 /stream 长轮询 ⇄ 本地桥（8375）⇄ WebGL 前端（Three.js + rete.js）；每端防守型 Sync Max FPS；首选项系统（General/Viewport/UI 分类，Preference.json 随场景保存）；统一颜色/属性系统。详见 devlog/README.md。
